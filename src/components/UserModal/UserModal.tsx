...
export const UserModal: React.FC<UserModalProps> = ({ isOpen, onClose }) => {
  ...
  const validateUsername = (name: string): string | null => {
    ...
    if (!trimmedName) return 'Имя пользователя не может быть пустым';
    if (trimmedName.length < 2) return 'Имя пользователя должно содержать минимум 2 символа';
    if (trimmedName.length > 20) return 'Имя пользователя не может быть длиннее 20 символов';
